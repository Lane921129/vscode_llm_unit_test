...
- 模型若對完全相同、可 assertion 的 Dynamic Trace 呼叫直接寫出 `assertEqual`、`assertTrue`、`assertFalse` 或 `assertIsNone`，其 assertion value 必須與該 Trace 相同；矛盾候選必須在執行前拒絕並以事實原因重試。此 gate 不得拒絕未 Trace 的候選輸入或經額外轉換後的 assertion。
