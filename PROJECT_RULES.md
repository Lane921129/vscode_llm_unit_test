...
- Python 相依補裝前必須列出綁定所選 interpreter、缺項、requirements／明確映射與工具相依的安裝清單，由使用者確認該清單後才執行。取消、關閉或沒有確認皆不得安裝；後續發現新缺項需另行確認，不永久記住安裝授權。requirements 或引用清單變更後必須重新預覽；未對應項目不得猜測套件名稱。清單不顯示套件來源憑證或任意 requirements 原文，直接宣告不冒充 pip 最終解析結果。
- 未對應的缺項必須提供直接補齊安裝名稱的入口。使用者可明確選擇同名 import 或填入套件名稱；保存映射後必須重建清單再確認安裝，保存名稱不可視為安裝授權。只允許修改目前清單的可編輯項目，不接受網址、路徑或 pip 引數，也不可覆蓋 requirements 版本限制。既有專案映射須合併保留。
