...
import { PythonInstallationPlan } from './pythonInstallationPlan';

export function installationPreviewHtml(plan: PythonInstallationPlan, nonce: string): string {
    const escape = (value: string) => value.replace(/[&<>"']/g, char => `&#${char.charCodeAt(0)};`);
    const rows = plan.missing.map(item => `<tr><td>${escape(item.module)}</td><td>${escape(item.installation)}</td><td>${item.locations.map(escape).join('<br>') || '單檔載入預檢'}</td></tr>`).join('');
...
${plan.blockers.length ? `<div class="blocker"><strong>請先處理以下項目，再重新檢查</strong><ul>${plan.blockers.map(value => `<li>${escape(value)}</li>`).join('')}</ul></div>` : ''}
...
<h2>將執行的安裝項目</h2><ul>${plan.operations.map(operation => `<li>${escape(operation.label)}</li>`).join('')}</ul>
...
<div class="actions"><button id="install" ${plan.blockers.length ? 'disabled' : ''}>確認並安裝</button><button id="cancel">取消，不安裝此清單</button></div>
<script nonce="${nonce}">
...
export async function confirmPythonInstallation(plan: PythonInstallationPlan, signal?: AbortSignal): Promise<boolean> {
    ...
    return new Promise<boolean>(resolve => {
        ...
        const finish = (approved: boolean) => {
            ...
            panel.dispose(); resolve(approved);
        };
